...
function addGroup(group) {
  if (!configured.value.some(item => item.id === group.username)) configured.value.push({ id: group.username, name: group.name, forward_to: [], system_prompt_file: 'prompts/class_group.md', enabled: true, history_limit: 30, aggregation_seconds: 8, importance_threshold: 70, chain: { enabled: false, rules: [] } })
...
function addChainRule() {
  selected.value.chain ||= { enabled: false, rules: [] }
  selected.value.chain.rules.push({ name: `接龙规则 ${selected.value.chain.rules.length + 1}`, enabled: true, match_keywords: [], exclude_keywords: [], entry_template: '', self_identifiers: [] })
}
function removeChainRule(index) { selected.value.chain.rules.splice(index, 1) }
function listText(values) { return (values || []).join('\n') }
function setList(rule, key, value) { rule[key] = [...new Set(value.split(/[\n,，]/).map(item=>item.trim()).filter(Boolean))] }
...
onMounted(async()=>{try{const [s,g,p]=await Promise.all([api.settings(),api.groups(),api.prompts()]);configured.value=JSON.parse(JSON.stringify(s.groups)).map(group=>({ ...group, chain: group.chain || { enabled: false, rules: [] } }));catalog.value=g;prompts.value=p;if(configured.value.length)selectedId.value=configured.value[0].id}catch(e){error.value=e.message}finally{loading.value=false}})
</script>
    ...
    <section class="workspace">
      ...
      <div v-if="selected" class="card editor-card">
        ...
        <div class="card-body">
          ...
          <div class="divider"></div><div class="section-title"><span>D</span><h4>自动接龙规则</h4></div>
          <div class="chain-head"><div class="field toggle-field"><label>允许本群自动接龙</label><select v-model="selected.chain.enabled" class="select"><option :value="false">关闭（默认）</option><option :value="true">启用，仅处理下方匹配规则</option></select></div><button class="btn small" type="button" @click="addChainRule">＋ 添加接龙规则</button></div>
          <p class="hint">未启用或未匹配任何规则的接龙绝不会自动参加。模板中的 <code v-pre>{{memory.key}}</code> 从全局共享记忆读取；缺失时 Agent 会先询问 forward_to。</p>
          <div v-if="selected.chain.rules.length" class="chain-rules">
            <article v-for="(rule,index) in selected.chain.rules" :key="index" class="chain-rule">
              <header><strong>规则 {{ index + 1 }}</strong><div class="actions"><label class="inline-check"><input v-model="rule.enabled" type="checkbox" />启用</label><button class="btn danger small" type="button" @click="removeChainRule(index)">删除</button></div></header>
              <div class="form-grid">
                <div class="field"><label>规则名称</label><input v-model="rule.name" class="input" placeholder="例如：宿舍晚点名" /></div>
                <div class="field"><label>接龙内容模板</label><input v-model="rule.entry_template" class="input mono" :placeholder="'例如：{{profile.dormitory}} {{profile.name}}已返校'" /><small>最终只生成一行，程序自动添加序号。</small></div>
                <div class="field"><label>必须包含的关键词（每行一个，全部匹配）</label><textarea class="textarea" rows="4" :value="listText(rule.match_keywords)" placeholder="晚点名&#10;宿舍" @input="setList(rule,'match_keywords',$event.target.value)"></textarea><small>不能为空；多条规则之间为任一匹配。</small></div>
                <div class="field"><label>排除关键词（每行一个，任一命中即跳过）</label><textarea class="textarea" rows="4" :value="listText(rule.exclude_keywords)" placeholder="无需接龙" @input="setList(rule,'exclude_keywords',$event.target.value)"></textarea></div>
                <div class="field span-2"><label>本人识别标记（每行一个）</label><textarea class="textarea mono" rows="3" :value="listText(rule.self_identifiers)" :placeholder="'{{profile.name}}'" @input="setList(rule,'self_identifiers',$event.target.value)"></textarea><small>若历史中的同一条目同时包含全部标记，即判定本人已经接过并跳过；即使不填，也会检查完整模板内容、本人发送方向和本地参与记录。</small></div>
              </div>
            </article>
          </div>
          <div v-else class="chain-empty">尚未配置接龙规则；即使总开关启用也不会参加任何接龙。</div>
        </div>
      </div>
